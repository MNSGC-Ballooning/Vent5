...
// Below: Check for GPS data, update buffer if data available
  GPS_LOCK = true; // tells you whether the GPS has a lock
  int numSats; // more GPS variables
...
// ^^^^^^^^^^ABOVE: Check for GPS data, update buffer if data available^^^^^^^^^^^^^
  ...
  if(currTime-prevTime>interval) // Interval is currently 2000 ms (or 2 seconds) as defined - this is the main loop/cycle
  {
    ...
    message = String(currTime/1000) + "," + String(heaterTempValue)+ "," + String(msPressure) + "," + String(pressureAltFeet); // the beginning of the message to be sent to our SD card. the ","'s (commas) are inserted as placeholders here since those variables weren't tracked/logged for this flight (see header to see which variables)
...
//
      if(abs(altFeet - prev_Control_Altitude) > 1000)
      {
        Serial.println("Bad GPS hit!");
        altFeet = prev_Control_Altitude + ascent_rate/0.3048*(millis() - prev_time2)/1000;
      } // use pressure alt?
//      
      ascent_rate = 0.3048*(((altFeet - prev_Control_Altitude)/(millis() - prev_time2))*1000); // calculate ascent rate and convert it to m/s
      
      avg_ascent_rate = 0; // reset before/after every loop, then calculated in the following for loop
      double j = 0;
      ...
      avg_ascent_rate = avg_ascent_rate/j; // basically sum up the last 10 ascent rates then divide by 10 to obtain the average ascent rate for this cycle 
      ...
      prev_time2 = millis(); // prev_time2 is only set when you get a fresh GPS hit
      prev_Control_Altitude = altFeet; // prev_Control_Altitude is only set when you get a fresh GPS hit
      GPSdata = "," + String(latitude, 4) + "," + String(lon, 4) + ","  + String(altFeet) + "," + String(numSats) + "," + String(ascent_rate); // record the GPS data into a string
  ...
  {
      altFeet = pressureAltFeet; // obtain the altitude (in feet) from the pressure sensor
      ...
      GPSdata = "," + String(latitude, 4) + "," + String(lon, 4) + ","  + String(altFeet) + "," + "0" + "," + String(ascent_rate); // latitude and lon remain unchanged here, and numSats is set to 0 to avoid an undeclared variable
      message += GPSdata; // add that GPS data string to the overall SD log message
 ...
 // *****************State Determination ("Worst-case" to "Best Case" Order of Operations)***************
    ...
    if(FlightHasBegun==false  && GPS_LOCK==true)
    {
      if(altFeet > 5000) // 5000 feet is the threshold to make sure that it doesn't prematurely vent or terminate when it's on the ground
    ...
    else if(FlightHasBegun==false && GPS_LOCK==false)
    {
...
//**************************************************************TERMINATION LOGIC BEGINS**************************************************************
    ...
    if(currTimeS > 240*60) // Master Backup timer (currently cuts 14,400 seconds (4 hours) after turning on in case all else fails)
    {
      terminate();
...
// ****INSERT ALTITUE GPS-BASED AND TIMER-BASED VENTING DECISIONS HERE****
    ...
    if(GPS_LOCK==true) // if the GPS has had a hit in the last minute or so, assume that its working (this if statement is the primary (GPS-based) state machine logic
    {
      ...
      if(altFeet > 80000) // This is a temporary venting to help ensure that the balloon stack will reach the desired altitude later in the flight
      {
      ...
      if(altFeet > 90000) // This is a second temporary venting to help ensure that the balloon stack will reach the desired altitude later in the flight
      {
      ...
      if(altFeet > 100000 || AlreadyReachedPermanentOpening) //Final vent
      {
...
// ABOVE IS FOR USE WITH THE SERIAL MONITOR ONLY**********************NOT FLIGHT-CRITICAL**********************
    ...
    message += "," + heaterStatus + ","+flapperState+","+String(servoPos())+","+String(cutterState)+","+stateSuggest+","+currentState+","+ventReason;
